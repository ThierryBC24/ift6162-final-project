...
class GlobalPlanner:
    ...
    def _theta_star(self, start: GridIndex, goal: GridIndex) -> List[GridIndex]:
        """
        Placeholder: for now, call A*.
        Later: implement true Theta* with line-of-sight shortcut checks
        using env.is_navigable along the segment.
        """
        return self._a_star(start, goal)

    def _reconstruct_path(self, came_from, current: GridIndex) -> List[GridIndex]:
        path = [current]
