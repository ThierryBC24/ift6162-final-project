"""
Minimal crossing COLREG demo (blank map) with GIF output.

Own: starts at (0,0), heading North, speed 5 m/s, holds course.
Traffic: starts at (500,0), heading West, speed 5 m/s, applies COLREGLogic (should give way).
"""
...
def main():
    own_state = VesselState(lat=0.0, lon=0.0, psi=np.pi / 2, v=5.0, x=0.0, y=-1000.0)
    own_params = VesselParams(max_yaw_rate=np.radians(20))
    own_ship = Vessel(own_state, own_params)

    traf_state = VesselState(lat=0.0, lon=0.0, psi=np.pi, v=5.0, x=600.0, y=0.0)
    ...
    ax.set_title("Crossing COLREG demo (blank map)")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ...
    anim.save("colreg_crossing_minimal.gif", fps=20)
    print("Saved colreg_crossing_minimal.gif")
    plt.close(fig)
