"""
Minimal head-on COLREG demo in a blank plane (no coastline load).

Own ship: starts at (0,0), heading East, speed 5 m/s.
Traffic ship: starts at (2000,0), heading West, speed 5 m/s.
COLREGLogic applied to traffic; own holds course.
"""
...
def main():
    ...
    # Traffic ship (head-on)
    traf_state = VesselState(lat=0.0, lon=0.0, psi=np.pi, v=5.0, x=2000.0, y=0.0)
    ...
    cfg = SimConfig(dt=1.0, t_final=600.0, log_interval=1)
    ...
    ax.set_title("Head-on COLREG demo (blank map)")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ax.set_aspect("equal", adjustable="box")
    ax.set_ylim(-200, 200)
    ax.set_xlim(-100, max(traf_x + own_x) + 100)
    ax.grid(True)
    ...
    anim.save("colreg_headon_minimal.gif", fps=20)
    print("Saved colreg_headon_minimal.gif")
    plt.close(fig)
