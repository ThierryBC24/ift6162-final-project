"""
Alternate overtaking COLREG demo (blank map) with GIF output.

Own/traffic swapped vs base overtaking demo; each takes the other's start and heading.
"""
...
def main():
    # Own starts where traffic was, with traffic's heading (East, faster lane)
    own_state = VesselState(lat=0.0, lon=0.0, psi=0.0, v=8.0, x=-2000.0, y=200.0)
    own_params = VesselParams(max_yaw_rate=np.radians(20))
    own_ship = Vessel(own_state, own_params)

    # Traffic starts where own was, with own's heading (East, slower lane)
    traf_state = VesselState(lat=0.0, lon=0.0, psi=0.0, v=4.0, x=0.0, y=0.0)
    ...
    ax.set_title("Overtaking COLREG demo (alt, blank map)")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ...
    anim.save("colreg_overtaking_minimal_alt.gif", fps=20)
    print("Saved colreg_overtaking_minimal_alt.gif")
    plt.close(fig)
